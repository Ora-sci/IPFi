...
use near_sdk::{
    assert_one_yocto, env, log, require, AccountId, Balance, BorshStorageKey, Gas, IntoStorageKey,
...
pub struct FungibleToken {
    ...
    /// Total supply of the ip token.
    pub total_supply: Balance,

    /// The storage size in bytes for one account.
    pub account_storage_usage: StorageUsage,

    pub permissions: LookupMap<Balance, AccessType>,
}
...
impl FungibleToken {
        ...
        S: IntoStorageKey,
    {
        let mut this = Self {
            ...
            permissions: LookupMap::new(StorageKey::Permissions.try_to_vec().unwrap()),
        };
    ...
    pub fn internal_grant_access(&mut self, account_id: &AccountId, access_type: AccessType) {
        let current_token = self.internal_unwrap_balance_of(account_id);
        self.permissions.insert(&current_token, &access_type);
    }
